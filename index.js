...
const { consolidateRBXM, createConsolidationSummary } = require('./consolidator');
...
// Consolidate multiple .rbxm files into a single ZIP and send webhook notification
app.post('/consolidate', async (req, res) => {
  try {
    ...
    if (experienceSecret) {
      const provided = req.headers['x-bridge-secret'] || req.headers['x-experience-secret'];
      if (!provided || provided !== experienceSecret) {
        Webhook.send('consolidate_rejected', { reason: 'invalid_experience_secret' });
        return res.status(401).json({ error: 'Invalid experience secret' });
      }
    }

    const { filePaths, assetIds, requesterIds, description } = req.body || {};
    ...
    const validFiles = filePaths.filter(fp => fs.existsSync(fp));
    if (validFiles.length === 0) {
      return res.status(404).json({ error: 'No valid .rbxm files found' });
    }

    Webhook.send('consolidate_started', { 
    ...
    // Consolidate files
    const { zipPath, manifestPath, manifest } = await consolidateRBXM(validFiles, {
      assetIds,
      requesterIds,
      description
    });

    // Send webhook with consolidated files
    const webhookDetails = {
      ...
      zipPath,
      manifestPath,
      assetIds: assetIds?.join(',') || 'none',
      requesterIds: requesterIds?.join(',') || 'none',
      description: description || 'Consolidated .rbxm files'
    };

    try { 
      Webhook.sendFile('consolidate_completed', zipPath, webhookDetails); 
    } catch (e) { 
      console.warn('Failed to send consolidate webhook:', e.message); 
    }
    ...
    Webhook.send('consolidate_error', { error: String(err?.message || err) });
    return res.status(500).json({ error: err?.message || 'consolidation failed' });
  }
});
